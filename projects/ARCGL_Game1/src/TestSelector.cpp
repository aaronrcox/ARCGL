...
TestSelectorState::TestSelectorState(Game1 *pApp) : TIGameState<Game1>(pApp)
{
...
{
	int mx = Input::Get()->GetMouseX();
	int my = Input::Get()->GetMouseY();

	if( mx > m_menuXPos && mx < m_menuXPos + m_menuWidth && my > m_menuYPos )
	{
		m_mouseOverTestID = (int)((my + (m_menuHeight*0.25f) - m_menuYPos) / m_menuHeight);
		if( m_mouseOverTestID >= (int)m_tests.size() )
			m_mouseOverTestID = -1;
	}
	else 
	{
	...
	if( Input::Get()->WasMouseButtonPressed(1) && m_mouseOverTestID != -1 )
	{
...
void TestSelectorState::Draw()
{
	...
	for(unsigned int i=0; i<m_tests.size(); i++)
	{
		...
		sb->DrawString(m_font, m_tests[i].c_str(), m_menuXPos + 25.0f, m_menuYPos + (i * m_menuHeight) );
	}

	sb->SetRenderColor(0x0000003F);
	sb->DrawSprite(NULL, m_menuXPos, 0, m_menuWidth, (float)GetApplication()->GetWindowHeight(), 0, 0, 0);
