...
static char gs_vertexShader_es[] =
"																								\r\n"
"																								\r\n"
...
static char gs_fragmentShader_es[] =
" "
#ifdef EMSCRIPTEN																					
...
void main() \r\n\
{ \r\n\
    if      ( v_textureID == 0.0 ) gl_FragColor = texture(u_texture0, v_texcoord.st) * v_color; \r\n\
    else if ( v_textureID == 1.0 ) gl_FragColor = texture(u_texture1, v_texcoord.st) * v_color; \r\n\
    else if ( v_textureID == 2.0 ) gl_FragColor = texture(u_texture2, v_texcoord.st) * v_color; \r\n\
    else if ( v_textureID == 3.0 ) gl_FragColor = texture(u_texture3, v_texcoord.st) * v_color; \r\n\
    else if ( v_textureID == 4.0 ) gl_FragColor = texture(u_texture4, v_texcoord.st) * v_color; \r\n\
...
SpriteBatch_GL3::SpriteBatch_GL3(Application *pApp) : SpriteBatch(pApp)
{
	...
	GLuint vshader = LoadShader(gs_vertexShader_es, GL_VERTEX_SHADER);
	GLuint pshader = LoadShader(gs_fragmentShader_es, GL_FRAGMENT_SHADER);
	...
	GetOrtho(m_projection, 0.0f, (float)pApp->GetWindowWidth(), (float)pApp->GetWindowHeight(), 0.0f, 0.0f, 100.0f);
...
//void SpriteBatch_GL3::DrawSprite(Texture *texture, glm::vec2 position, const glm::vec2 &size)
{
	...
	//auto iter = m_activeTextures.find(texture);
...
void SpriteBatch_GL3::DrawLine(float x1, float y1, float x2, float y2, float lineThickness)
{
	float xDiff = x2 - x1;
	float yDiff = y2 - y1;
	...
	DrawSprite(m_nullTexture, x1, y1, len, lineThickness, rot, 0.0f, 0.5f);

	SetUVRect(uvX, uvY, uvW, uvH);

}

void SpriteBatch_GL3::DrawString(Font *pFont, const char *string, float xPos, float yPos)
{
...
unsigned int SpriteBatch_GL3::PushTexture(Texture *texture)
{
	...
	char texAttrib[32];
	sprintf(texAttrib, "u_texture%i", m_currentTextureID);
...
////void main() \r\n\
////{ \r\n\
////    if      ( v_textureID == 0.0 ) out_color = texture(u_texture0, v_texcoord.st) * v_color; \r\n\
////    else if ( v_textureID == 1.0 ) out_color = texture(u_texture1, v_texcoord.st) * v_color; \r\n\
////    else if ( v_textureID == 2.0 ) out_color = texture(u_texture2, v_texcoord.st) * v_color; \r\n\
////    else if ( v_textureID == 3.0 ) out_color = texture(u_texture3, v_texcoord.st) * v_color; \r\n\
////    else if ( v_textureID == 4.0 ) out_color = texture(u_texture4, v_texcoord.st) * v_color; \r\n\
////    else out_color = vec4(1.0, 1.0, 1.0, 1.0); \r\n\
...
//void main() \r\n\
//{ \r\n\
//	out_color = texture(u_texture[v_textureID], v_texcoord.st) * v_color; \r\n\
//	\r\n\
//}";
//
//// helper function defined below
...
//	glBindVertexArray(0);
//
//	GetOrtho(m_projection, 0.0f, pApp->GetWindowWidth(), pApp->GetWindowHeight(), 0.0f, 0.0f, 100.0f);
//
//}
//
//SpriteBatch_GL3::~SpriteBatch_GL3()
...
//
//
//	for(int i=0; i<m_currentTextureID; i++)
//		m_activeTextures[i] = nullptr;
//
//	m_numFlushes+= 1;
//
...
//	unsigned int viewHeight = m_pApplication->GetWindowHeight();
//
//	GetOrtho(m_projection, 0.0f, viewWidth, viewHeight, 0.0f, -1.0f, 100.0f);
//
//	glUseProgram( m_shader );
//
//	glUniformMatrix4fv( glGetUniformLocation(m_shader, "u_projection"), 1, false, m_projection );
...
////void SpriteBatch_GL3::DrawSprite(Texture *texture, glm::vec2 position, const glm::vec2 &size)
//{
//	if( texture == NULL ) return; //texture = m_null_texture;
//
//	if( ShouldFlush() ) FlushBatch();
//	unsigned int textureID = PushTexture( texture );
//
...
//	m_vertices[m_currentVert].pos[0] = xPos + tlX; 
//	m_vertices[m_currentVert].pos[1] = yPos + tlY; 
//	m_vertices[m_currentVert].pos[2] = textureID;
//	m_vertices[m_currentVert].color[0] = r;
//	m_vertices[m_currentVert].color[1] = g;
//	m_vertices[m_currentVert].color[2] = b;
//	m_vertices[m_currentVert].color[3] = a;
...
//}
//
//void SpriteBatch_GL3::DrawLine( float x1, float y1, float x2, float y2 )
//{
//	float xDiff = x2 - x1;
//	float yDiff = y2 - y1;
//	float len = sqrt( xDiff * xDiff + yDiff * yDiff );
...
//	SetUVRect(0.0f, 0.0f, 1.0f, 1.0f);
//
//	DrawSprite(m_nullTexture, x1, y1, len, m_lineThickness, rot, 0.0f, 0.5f);
//
//	SetUVRect(uvX, uvY, uvW, uvH);
//
//}
...
//	}
//
//	//bool containsKey = m_activeTextures.ContainsKey(texture);
//	//bool containsKey =  m_activeTextures.find(texture) != m_activeTextures.end();
//	//if (containsKey == false)
//
//
//
//	//auto iter = m_activeTextures.find(texture);
//	//if( iter == m_activeTextures.end() )
//	{
//		if (m_currentTextureID >= m_maxTextures-1)
//			FlushBatch();
//
