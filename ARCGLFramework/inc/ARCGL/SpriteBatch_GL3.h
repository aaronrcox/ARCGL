/*-----------------------------------------------------------------------------
Author:			Aaron Cox
Description:	Implements Non Immediate mode rendering
...
private:
	...
	void GetOrtho(float *mat4x4, float left, float right, float bottom, float top, float a_fNear, float a_fFar)
	{
		...
		mat4x4[0] = 2.0/(right - left);;
		mat4x4[1] = 0.0f;
		mat4x4[2] = 0.0f;
		mat4x4[3] = 0.0f;

		mat4x4[4] = 0.0f;
		mat4x4[5] = 2.0/(top - bottom);
		mat4x4[6] = 0.0f;
		mat4x4[7] = 0.0f;
		
		mat4x4[8] = 0.0f;
		mat4x4[9] = 0.0f;
		mat4x4[10] = 2.0/(a_fFar - a_fNear);
		mat4x4[11] = 0.0f;

		mat4x4[12] = -1*((right + left)/(right - left));
		mat4x4[13] = -1*((top + bottom)/(top - bottom));
		mat4x4[14] = -1*((a_fFar + a_fNear)/(a_fFar - a_fNear));
		mat4x4[15] = 1;
	}
