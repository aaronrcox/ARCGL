...
Test_SpriteBatch_FillRate::Test_SpriteBatch_FillRate(Game1 *pApp) : TIGameState<Game1>(pApp)
{
...
{
	int w = GetApplication()->GetWindowWidth();
	int h = GetApplication()->GetWindowHeight();

	m_sprites.push_back( Point( (float)(rand() % w), (float)(rand() % h) ) );
	m_numSprites += 1;
}
