...
Test_SpriteBatch::Test_SpriteBatch(Game1 *pApp) : TIGameState<Game1>(pApp)
{
...
void Test_SpriteBatch::Draw()
{
	...
	unsigned int windowWidth = GetApplication()->GetWindowWidth();
	unsigned int windowHeight = GetApplication()->GetWindowHeight();

	float left		= 64;
	float right		= windowWidth - 64.0f;
	float top		= 64;
	float bottom	= windowHeight - 64.0f;
	float hCenter	= windowWidth / 2.0f;
	float vCenter	= windowHeight / 2.0f;
